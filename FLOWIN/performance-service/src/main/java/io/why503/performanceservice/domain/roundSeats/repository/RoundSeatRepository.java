...
public interface RoundSeatRepository extends JpaRepository<RoundSeatEntity,Long> {
    ...
    //특정 회차의 모든 좌석 조회
    List<RoundSeatEntity> findByRoundSq_RoundSq(Long roundSq);

    //특정 회차의 특정 상태인 좌석만 조회
    List<RoundSeatEntity> findByRoundSq_RoundSqAndRoundSeatStatus(Long roundSq, RoundSeatStatus roundSeatStatus);
