...
import io.why503.accountservice.domain.companies.model.dto.req.CompanyEmailVerifyReqDto;
import io.why503.accountservice.domain.companies.model.dto.res.CompanyEmailVerifyResDto;
import io.why503.accountservice.domain.companies.sv.impl.CompanyEmailVerifySvImpl;
...
public class CompanyEmailVerifyCtrl {
    ...
    public ResponseEntity<CompanyEmailVerifyResDto> verifyEmail(
        ...
        accountService.updateUserRoleBySq(sq, UserRole.COMPANY);
        return ResponseEntity.ok(
                new CompanyEmailVerifyResDto(
                        true, // 인증 성공
                        "이메일 인증 성공." // 인증 성공 메시지
